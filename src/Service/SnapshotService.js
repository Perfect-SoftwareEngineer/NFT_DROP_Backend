...
class SnapshotService{
  ...
  async setBbHolderData (response) {
    try{
      let results = [];
      const data = await this.getHolder(137, process.env.DROP1_ADDRESS);
  ...
  async setSerumHolderData (request, response) {
    try{
  ...
  async setBbCommunityHolderData (request, response) {
    const data = request.files.file.data;
    const options = {
      delimiter: ',', // optional
  ...
  async setSerumCommunityHolderData (request, response) {
    const data = request.files.file.data;
    const name = request.files.file.name.replace('.csv', '');
    const tokenId = this.serumId[name];
    if(!tokenId) {
  ...
  async setIntelHolderData (drop, response) {
    try{
        
        let results = [];

        const data = await this.getHolderByTokenId(137, process.env.INTEL_ADDRESS, 1);
        results = results.concat(data);
  ...
  async getHolder (network, token_address) {
    let results = [];
    let progress = true;
    let cursor = "";
    let chain;
  ...
  async getHolderByTokenId (network, token_address, tokdnId) {
    let results = [];
    let progress = true;
    let cursor = "";
    let chain;
