...
describe("MetadataD1", () => {
  beforeAll(async () => {
    ...
    await supertest(app).post('/api/metadata/drop1/create').send(payload1);
    await supertest(app).post('/api/metadata/drop1/create').send(payload2);
  });
  ...
  describe("get metadata by id", () => {
    describe("given the metadata does not exist", () => {
      it("should return a empty array", async () => {
        ...
        const { body, statusCode } = await supertest(app).get(`/api/metadata/drop1/get/${tokenId}`);
  ...
  describe("get all metadata", () => {
    it("should return a 200 status and all the metadata", async () => {
      ...
      const { body, statusCode } = await supertest(app).get(`/api/metadata/drop1/getAll`);
      expect(statusCode).toBe(200);
  ...
  describe("create metadata", () => {
    describe("given the metadata does exist", () => {
      it("should return a 500 status", async () => {
        await supertest(app).post('/api/metadata/drop1/create').send(payload1).expect(500);
      });
    });

    describe("given the metadata does not exist", () => {
      it("success to create metadata", async () => {
        await supertest(app).post('/api/metadata/drop1/create').send(payload3).expect(200);
        const { body, statusCode } = await supertest(app).get(`/api/metadata/drop1/get/3`);
        expect(statusCode).toBe(200);
        expect(body).toEqual({
